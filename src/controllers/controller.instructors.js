...
async function create(req, res){
    ...
    if(validate.error) return res.status(202).json({Error: validate.error})
...
async function edit(req, res){
    ...
    if(peticion.error) return res.status(404).json(peticion.error)
...
async function remove(req, res){
    ...
    return res.status(200).json({Removed: peticion})
}
